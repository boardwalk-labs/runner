...
>
> Do not confuse with the flagship engine: the flagship is the _entire control plane on your hardware, no Boardwalk involvement_; this runner is _Boardwalk-scheduled work executing on your machines_. Different users, different trust model.
...
- Workspace, credentials, artifacts, and logs are isolated per run and cleaned on completion; cross-run leakage is a P0 bug.
...
- Artifact upload + run-event/log streaming clients (the same wire format as everywhere).
...
3. Contract types published and consumed by at least the platform implementation; publication checklist passes.
